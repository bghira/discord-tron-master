...
class WorkerManager:
    ...
    def find_best_fit_worker(self, job_type: str) -> str:
        ...
        for worker_id in self.workers_by_capability[job_type]:
            worker_hardware_limits = self.get_worker_hardware_limits(worker_id)
            worker_hardware = self.workers[worker_id]["hardware"]
            if worker_hardware_limits["gpu"] >= worker_hardware["gpu"] and worker_hardware_limits["compute"] >= worker_hardware["compute"] and worker_hardware_limits["memory"] >= worker_hardware["memory"]:
                return worker_id
        pass

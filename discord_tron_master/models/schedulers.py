...
class Schedulers(db.Model):
    ...
    def set_use_case(name, use_case):
        ...
        if type_is != "list" or type_is != "dict":
            raise ValueError(
                f"Received {type_is} for use_case record method, and needed list or dict"
            )
    ...
    def create(
        ...
        steps_range_begin: int = 10,
        steps_range_end: int = 500,
    ):
        existing_definition = Schedulers.get_by_name(name)
        if existing_definition is not None:
            db.session.delete(existing_definition)
        ...
        scheduler = Schedulers(
            ...
            steps_range_begin=steps_range_begin,
            steps_range_end=steps_range_end,
        )

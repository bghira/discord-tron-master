...
class UserHistory(db.Model):
    ...
    def get_all_user_prompts(user: str):
        return (
            UserHistory.query.with_entities(UserHistory.prompt)
            .filter_by(user=user)
            .distinct()
            .all()
        )
    ...
    def add_entry(user: str, message: str, prompt: str, config_blob: dict = {}):
        result = UserHistory.create(
            user, message, prompt, config_blob=json.dumps(config_blob)
        )
    ...
    @staticmethod
    def get_user_most_common_terms(
        user_history, term_limit: int = 10, search_limit: int = 10000
    ) -> dict:
        """
        ...
        for entry in user_history:
            ...
            if not entry.prompt:
                logging.warning(
                    f"Entry {entry} had no prompt. Not including in statistics."
                )
                continue

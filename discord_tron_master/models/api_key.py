...
class ApiKey(db.Model):
    ...
    def generate_by_user_id(user_id):
        ...
        if client is None:
            client = OAuthClient(user_id=user_id, client_id=OAuthClient.generate_client_id(), client_secret=OAuthClient.generate_client_secret())
            db.session.add(client)
            db.session.commit()
        api_key = ApiKey(api_key=ApiKey.generate_api_key(), client_id=client.client_id, user_id=user_id)

...
class Transformers(db.Model):
    ...
    def get_by_full_model_id(full_model_id):
        model_id = full_model_id.split("/")[1]
        model_owner = full_model_id.split("/")[0]
        return Transformers.query.filter_by(
            model_id=model_id, model_owner=model_owner
        ).first()
    ...
    def create(
        ...
        recommended_positive: str = None,
        recommended_negative: str = None,
        tags: str = None,
    ):
        existing_definition = Transformers.query.filter_by(model_id=model_id).first()
        if existing_definition is not None:
        ...
        transformer = Transformers(
            ...
            recommended_positive=recommended_positive,

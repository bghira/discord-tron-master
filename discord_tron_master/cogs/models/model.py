...
class Model(commands.Cog):
    ...
    @commands.command(
        name="model-list", help="List all models currently approved for use."
    )
    async def model_list(self, ctx):
        ...
        with app.app_context():
            ...
            for transformer in pre_filtration_transformers:
                if (
                    f"{transformer.model_owner}/{transformer.model_id}".lower()
                    not in allowed_models
                    and allowed_models != []
                ):
                    logging.info(
                        f"Removing {transformer.model_owner}/{transformer.model_id} from allowed model list, as, we have allowed models set."
                    )
                else:
                ...
                idx += 1
            logging.debug(
                f"Transformers, post-filtration: {pre_filtration_transformers}"
            )
        ...
        def build_transformer_output(transformer):
            cluster = (
                f"{transformer.model_owner}/{transformer.model_id}: {transformer.description}\n"
                f"!model {transformer.model_owner}/{transformer.model_id}\n"
            )
            return cluster

        def split_into_chunks(
            text_lines: List[str], max_length: int = 2000
        ) -> List[str]:
            chunks = []
            current_chunk = ""
            for line in text_lines:
                if (
                    len(current_chunk) + len(line) + len(wrapper) + len(wrapper)
                    > max_length
                ):
    ...
    @commands.command(
        ...
        help="Are you an image admin? Set a description for your model.",
    )
            ...
            if role.name == "Image Admin":
                ...
                with app.app_context():
                    ...
                    if not transformer:
                        ...
                        message = "Successfully set the new description for " + str(
                            model_id
                        )
        await ctx.send(message)
    ...
    @commands.command(
        name="model-allow",
        help="Allow a model from the list of available models. (Admin only)",
    )
    ...
    async def model(self, ctx, full_model_name: str = None):
        ...
        if "/" not in full_model_name:
            ...
            return
        with app.app_context():
            new_model_id, new_model_owner = (
                full_model_name.split("/")[1],
                full_model_name.split("/")[0],
            )
            existing = Transformers.query.filter_by(
                model_id=new_model_id, model_owner=new_model_owner
            ).first()
        ...
        if (
            ...
            and full_model_name
            and allowed_models != []
            and f"{new_model_owner}/{new_model_id}".lower() not in allowed_models
        ):
            await ctx.send(
                "That model registered for use, but this server's administrator does not want it available. Admins can use `!model-allow <model>` to enable it."
            )
            return

        if not existing:
            await ctx.send(
                "That model is not registered for use. To make your 'awesome' images, or whatever, have an admin use `!model-add <model> <image|text> <description>` where `image|text` determines whether it's a diffuser or language model."
            )
            return
    ...
    @commands.command(
        name="model-delete", help="Delete a model. Not available to non-admins."
    )
    async def model_delete(self, ctx, full_model_name: str):
        ...
        try:
            with app.app_context():
                ...
                await ctx.send(
                    f"Sigh. Well, it is done. That model is now obliviated from existence."
                )
        except Exception as e:
            logging.error(f"Could not delete model: {e}")
            await ctx.send(
                f"Sorry bae, could not delete that model for you. Have you tried using more lube? {e}"
            )

    @commands.command(name="model-add", help="Add a model to the list for approval.")
    async def model_add(
        self, ctx, full_model_name: str, model_type: str, *, description
    ):
        if not guild_config.is_guild_home(ctx.guild.id):
            await ctx.send("sorey bae we are not in Kansas anymore.")
            return
        ...
        with app.app_context():
            ...
            if existing:
                await ctx.send(
                    "That model is already registered. Go use it to make your horse cock porn, or whatever!"
                )
                return
        ...
        try:
            ...
            logging.error(
                f"Error while attempting to list model details: {e}, traceback: {traceback.format_exc()}"
            )
            await ctx.send(
                f"I am very sorry. That model was not found. I searched for it at, https://huggingface.co/api/models/{full_model_name}."
            )
            return
        user_id = ctx.author.id
        ...
        try:
            ...
            logging.error(
                f"Error while attempting to create new record: {e}, traceback: {traceback.format_exc()}"
            )
            await ctx.send(
                f"I am very sorry. I was unable to create a new record for that model. You might be touching yourself at night too frequently. Have you tried touching yourself less?"
            )
            return
        await ctx.send(
            "That model exists? Cool. I'll add it to the list for approval. Please be patient. I'm a bot, and I'm slow. I'll let you know when it's approved."
        )

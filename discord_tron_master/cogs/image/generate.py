...
class Generate(commands.Cog):
    ...
    @commands.command(
        name="generate-random-x",
        help="Generates images based on a random prompt, x number of times at once.",
    )
    async def generate_range_random(self, ctx, arg_count=None, *, theme=None):
        ...
        if worker is None:
            discord_first_message = await ctx.send(
                "No workers available. Image was **not** added to queue. 😭 aw, how sad. 😭"
            )
        ...
        if arg_count is None:
            ...
            has_been_warned_about_count_being_digit = user_config.get(
                "has_been_warned_about_count_being_digit", False
            )
            if not has_been_warned_about_count_being_digit:
                await ctx.send(
                    "Count must be a number. I assume you meant 3 images. Here you go! You'll never see this warning again. It's a sort of 'fuck you'."
                )
                self.config.set_user_setting(
                    ctx.author.id, "has_been_warned_about_count_being_digit", True
                )
            count = 3
        out_theme = ""
    ...
    @commands.command(
        name="generate-x",
        help="Generates an image based on the given prompt, x number of times at once.",
    )
    async def generate_range(self, ctx, count, *, prompt):
        ...
        if not count.isdigit():
            ...
            if not has_been_warned_about_count_being_digit:
                await ctx.send(
                    "Count must be a number. I assume you meant 3 images. Here you go! You'll never see this warning again. It's a sort of, '*au revoir*'."
                )
                self.config.set_user_setting(
                    ctx.author.id, "has_been_warned_about_count_being_digit", True
                )
    ...
    @commands.command(
        name="compare",
        help="Generate an SD3 and DALLE3 image, side by side for comparison.",
    )
    ...
    @commands.command(
        name="dalle", help="Generates an image based on the given prompt using DALL-E."
    )
    async def generate_dalle(self, ctx, *, prompt):
        ...
        gpt = GPT()
        image_output = await gpt.dalle_image_generate(
            prompt=prompt,
            user_config=self.config.get_user_config(user_id=ctx.author.id),
        )
        await ctx.channel.send(
            file=discord_lib.File(BytesIO(image_output), "image.png")
        )

    @commands.command(
        name="sd3",
        help="Generates an image based on the given prompt using Stable Diffusion 3.",
    )
    async def generate_sd3(self, ctx, *, prompt):
        if guild_config.is_channel_banned(ctx.guild.id, ctx.channel.id):
            logging.warning(
                "Not generating image in channel. Channel is banned from gen."
            )
            return
    ...
    @commands.command(
        name="generate", help="Generates an image based on the given prompt."
    )
    async def generate(self, ctx, *, prompt):
        ...
        if (
            ...
            await DiscordBot.send_large_message(
                ctx=ctx,
                text=f"{ctx.author.mention}: You can use `--multiline` or `!multiline` as a flag to use the prompt as-is. I will split your prompt into multiple images without that.",
            )
            ...
            try:
                image = stabilityai.generate_image(
                    prompt, user_config, model="sd3-turbo"
                )
                await ctx.channel.send(
                    file=discord_lib.File(BytesIO(image), "image.png")
                )
            except Exception as e:
        ...
        for _prompt in prompts:
            try:
                ...
                discord_first_message = await DiscordBot.send_large_message(
                    ctx=ctx, text=f"Job queuing: `" + _prompt + "`"
                )
                job = ImageGenerationJob(
                    ctx.author.id,
                    (self.bot, self.config, ctx, _prompt, discord_first_message),
                ...
                with app.app_context():
                    try:
                        user_history = UserHistory.add_entry(
                            user=ctx.author.id,
                            message=int(
                                f"{ctx.id if hasattr(ctx, 'id') else ctx.message.id}{idx}"
                            ),
                            prompt=_prompt,
                            config_blob=extra_payload["user_config"],
                        )
                        idx += 1
                    except Exception as e:
                        logger.warning(
                            f"Had trouble adding the user history entry: {e}"
                        )
                # Generate a "Job" object that will be put into the queue.
                await discord_first_message.edit(
                    content=f"Job {job.id} queued on {worker.worker_id}: `"
                    + _prompt
                    + "`"
                )
    ...
    async def get_statistics(self, ctx, user_id=None):
        ...
        try:
            ...
            with app.app_context():
                ...
                try:
                    gpt = GPT()
                    sentiment_analysis = await gpt.sentiment_analysis(
                        UserHistory.get_user_most_common_prompts(
    ...
    @commands.command(
        name="search",
        help="Search for a prompt in your history. Supports wildcards (*, ?) and exclusion via --not TERM.",
    )
        ...
        try:
            ...
            with app.app_context():
                ...
                discovered_prompts = UserHistory.search_all_prompts(
                    search_term=included_str, excludes=excludes
                )
            filtered_prompts = []
    ...
    @commands.command(
        name="random",
        help="Search for random prompts. Returns up to 10 prompts, unless a number is provided.",
    )
    async def random_prompts(self, ctx, count: int = 10, raw: str = None):
        try:
            ...
            with app.app_context():
                ...
                if raw is None:
                    output_string = (
                        f"{ctx.author.mention} I found {found_string} for you:"
                    )
                else:
                    output_string = ""
    ...
    @commands.command(
        name="invite", help="Invites the user to the latest thread in the channel."
    )
    async def invite_to_thread(self, ctx):
        ...
        try:
            ...
            await thread.send(f"{ctx.author.mention} is now in the party.")
        except Exception as e:

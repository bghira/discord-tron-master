...
class User(commands.Cog):
    ...
    async def clear_history(self, ctx):
        ...
        try:
            ...
            logging.error(
                "Caught error when clearing user conversation history: " + str(e)
            )
            self._send_generic_error(ctx)

    @commands.command(
        name="styles", help="Display some style template prompts you can select from."
    )
    ...
    @commands.command(
        name="style",
        help="Display or set your style template. Can be overridden with `--style` in a prompt.",
    )
    async def manage_style(self, ctx, style_name=None):
        try:
            ...
            await ctx.send(f"{ctx.author.mention} {self.generic_error}.")

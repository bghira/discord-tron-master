...
from discord_tron_master.classes.jobs.stablelm_prediction_job import (
    StableLMPredictionJob,
)
...
class Stablelm_predict(commands.Cog):
    ...
    @commands.command(
        name="stablelm", help="Generates an image based on the given prompt."
    )
        ...
        try:
            ...
            if worker is None:
                await discord_first_message.edit(
                    content="No workers available. StableLM query was **not** added to queue. 😭 aw, how sad. 😭"
                )

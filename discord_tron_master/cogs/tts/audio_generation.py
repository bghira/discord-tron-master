...
class Audio_generation(commands.Cog):
    ...
    VOICES = {
        ...
        "en_deadpan": "en_deadpan",
        "en_female_intense": "en_female_intense",
        "en_german_professor": "en_german_professor",
        "en_interesting_tone": "en_interesting_tone",
        "en_male_nervous_subdued": "en_male_nervous_subdued",
        "en_male_professional_reader": "en_male_professional_reader",
        "en_man_giving_ted_talk": "en_man_giving_ted_talk",
        "en_narrator_deep": "en_narrator_deep",
        "en_narrator_light_bg": "en_narrator_light_bg",
        "en_old_movie_actor": "en_old_movie_actor",
        "en_public_speaker_2": "en_public_speaker_2",
        "en_public_speaker": "en_public_speaker",
        "en_quiet_intense": "en_quiet_intense",
        "en_smooth_gruff": "en_smooth_gruff",
        "en_solo_singer": "en_solo_singer",
        "en_tv_commercial": "en_tv_commercial",
        "en_female_professional_reader": "en_female_professional_reader",
        "en_female_storyteller": "en_female_storyteller",
        ...
        "sing2": "sing2",
        "sing_3": "sing_3",
        "snarky_but_noisy": "snarky_but_noisy",
        "snarky_narrator_but_noisy": "snarky_narrator_but_noisy",
        "speaker_2": "speaker_2",
        "speaker_3": "speaker_3",
        "talkradio": "talkradio",
    ...
    @commands.command(
        name="tts", help="Generates an audio file based on the given prompt."
    )
        ...
        try:
            ...
            if worker is None:
                await discord_first_message.edit(
                    content="No workers available. TTS request was **not** added to queue. 😭 aw, how sad. 😭"
                )
    ...
    @commands.command(
        name="tts-voice", help="Set a voice for your whole TTS experience."
    )
    ...
    @commands.command(
        name="actors",
        help="Configure specific voices for characters, allowing you to use {CHARACTER_NAME_HERE} as a string .",
    )
    ...
    def list_available_languages(self, languages=None):
        ...
        max_columns = 5
        language_blocks = [
            dict(list(grouped_languages.items())[i : i + max_columns])
            for i in range(0, len(grouped_languages), max_columns)
        ]
        ...
        for block in language_blocks:
            ...
            header_row = (
                "| "
                + " | ".join(
                    prefix.ljust(max_field_widths[prefix]) for prefix in block.keys()
                )
                + " |\n"
            )
            ...
            separator_row = (
                "+-"
                + "-+-".join("-" * max_field_widths[prefix] for prefix in block.keys())
                + "-+\n"
            )
    ...
    def list_actors(self, actors: dict):
        ...
        for actor in actors:
            ...
            output = (
                output
                + f"                   To use it in a prompt, on its own line, type"
                + " {`"
                + f"{actor}"
                + "}`: Hello, world!\n"
            )
